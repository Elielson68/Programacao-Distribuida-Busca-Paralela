...
namespace Estudo.Threads
{
    ...
    class FindFiller
    {
        ...
        static string dontFindBecause = "";
        static string FindedFileText = "";
        /// <summary>
        ...
        /// </summary>
        public static string Busca(string[] args, bool useThreadSearch=true)
        {
            if(args.Length < 2)
            {
                TextoColorido.Imprimir("(B=Red|L=Yellow)Argumentos path e file são necessários!(.)\n");
                return "Argumentos path e file são necessários!";
            }
            string pattern = @"(.+)=(.+)";
            for(int i=0; i<args.Length; i++)
            {
                ...
                if(arg.Groups[1].Value is "" || arg.Groups[2].Value is "")
                {
                    ...
                    return $"O argumento {i+1} está vazio ou incorreto!";
                }
                else if (!DicOptions.Keys.Contains(arg.Groups[1].Value))
                {
                    ...
                    return "A chave \"{arg.Groups[1].Value}\" não é válida!";
                }
                else
                {
            ...
            {
                InitSearch(path:DicOptions["path"], fileName:DicOptions["file"],
                                             typeFile: DicOptions["extension"], dateFile: DicOptions["date"],
                                             lengthFile: DicOptions["bytes"], wordOrPhraseIncluse: DicOptions["content"], useThread: useThreadSearch);
            }
            ...
            string message = $"Tempo de procura: {stopwatch.Elapsed}\n"+FindedFileText+"\n";
            if (!fileIsFinded)
                message += dontFindBecause;
            return message;
        }
        /// <summary>
        ...
        static void PrintLogSearch(string fileToFind)
        {
            if(!fileIsFinded)
            {
                ...
                dontFindBecause += $"O arquivo {fileToFind} não foi encontrado!\n";
                if(!typeFileIsFinded && DicOptions["extension"] is not "")
                {
                    ...
                    dontFindBecause += $"Não foi encontrado o tipo de arquivo especificado\n";
                }
                if(!dateIsFinded && DicOptions["date"] is not "")
                {
                    ...
                    dontFindBecause += $"Não foi encontrado a data do arquivo especificado\n";
                }
                if(!lengthIsFinded && DicOptions["bytes"] is not "")
                {
                    ...
                    dontFindBecause += $"Não foi encontrado o tamanho do arquivo especificado\n";
                }
                if(!wordOrPhraseIsFinded && DicOptions["content"] is not "")
                {
                    ...
                    dontFindBecause += $"Não foi encontrada a palavra ou frase especificada\n";
                }
            }
        }
        /// <summary>
                ...
                if(finded)
                {
                    ...
                    if(wordOrPhraseIncluse is not "" && !wordOrPhraseIsFinded)
                    {
                        ...
                        catch
                        {
                            ...
                            dontFindBecause += $"O arquivo {f.Name} foi tentado ser aberto porém algo de errado ocorreu!\n";
                        }
        ...
        /// <param name="wordOrPhraseIncluse">Parametro opcional: verifica uma palavra ou frase existe dentro do arquivo</param>
        /// </summary>
        static void InitSearch(string path, string fileName, string typeFile="", string dateFile="", string lengthFile="", string wordOrPhraseIncluse="", bool enableLog=false, bool useThread=true)
        {
                ...
                try
                {
                    ...
                    else
                    {
                        foreach(string item in directories)
                            InitSearch(item, fileName, typeFile, dateFile, lengthFile, wordOrPhraseIncluse, enableLog, useThread);
                    }
                }
                catch
                {
        ...
        public static void ResetValues()
        {
            fileIsFinded = false;
            fileIsFinded = false;
            typeFileIsFinded = false;
            dateIsFinded = false;
            lengthIsFinded = false;
            wordOrPhraseIsFinded = false;
            dontFindBecause = "";
            FindedFileText = "";
            DicOptions = new Dictionary<string, string>(){
            {"path", ""},
            {"file", ""},
            {"extension", ""},
            {"date", ""},
